...
add_odd_equation(problem, "dt(V) - ν*lap(V,Vz) + + ν*k*k*V + lift(tau_v4) = -dy(P) - u*k*V - V*dy(v) - W*dz(v) + F*U")
add_even_equation(problem, "dt(w) - ν*lap(w,wz) + dz(p) - b + lift(tau_w2) = - v*dy(w) - w*dz(w) - 0.5*U*W*k - 0.5*V*dy(W) - 0.5*W*dz(W)")
add_even_equation(problem, "dt(W) - ν*lap(W,Wz) + ν*k*k*W - bp +dz(P) + lift(tau_w4) = - u*W*k - V*dy(w) - W*dz(w)")
