...
dist = d3.Distributor(coords, dtype=dtype, mesh=(1, MPI.COMM_WORLD.size)) # avoid distributing over small x direction
xbasis = d3.RealFourier(coords['x'], size=4, bounds=(0, Lx), dealias=dealias) # need 4 for single mode (don't worry)
...
p = even_field(dist, name='p', bases=(xbasis, ybasis, zbasis))
b = even_field(dist, name='b', bases=(xbasis, ybasis, zbasis))
u = even_field(dist, name='u', bases=(xbasis, ybasis, zbasis))
v = odd_field(dist, name='v', bases=(xbasis, ybasis, zbasis))
w = even_field(dist, name='w', bases=(xbasis, ybasis, zbasis))
...
CFL.add_velocity(u*ex + v*ey + w*ez)
