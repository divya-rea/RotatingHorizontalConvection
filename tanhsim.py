...
Ny, Nz = 32, 64 # horizontal, vertical resolution
f_rolloff = 4 # gridpoints resolving rolloff of f (dont change)
dealias = 3/2
dtype = np.float64
stop_sim_time = 200000 / f0
max_timestep = 0.01 / f0 # need to explicitly resolve rotation (origionally 0.2 / f0)
...
B['g'] = - b0 * np.cos(2*np.pi*y/Ly)
F = even_field(dist, name='F', bases=ybasis) # Mollified Coriolis parameter
F['g'] = (-f0/2) *(1 - np.tanh((2*y)-5))

# Check meridional profile of f and b
fig = plt.figure()
ax = plt.axes()
plt.plot(F['g'][:,0], color='k', label="f profile")
plt.plot(B['g'][:,0], color='r', label='Surface buoyancy')
plt.legend()
plt.grid()
ax.set_xticks(np.linspace(0,63,11))
ax.set_xticklabels(np.linspace(-10,10,11))
plt.xlim(0,63)
plt.xlabel('Y')
